...
import { DentalViewer3D } from './components/DentalViewer3D/DentalViewer3D';
...
export function App() {
  ...
  // Navegação
  const [activeTab, setActiveTab] = useState<'CASES' | 'DENTISTS' | 'TEAM' | 'CONFIG' | 'DEMO_3D'>('CASES');
  ...
  const handleAcceptCase = (caseId: string) => {
    ...
    const updatedCase: DentalCase = {
      ...targetCase,
      status: 'PENDING_APPROVAL' // Planejamento aceito e iniciado / aguardando aprovação
    };
  ...
  return (
    <div className="min-h-screen bg-cyber-bg text-slate-100 flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        <div className="p-5 rounded-2xl glass-panel border border-cyber-border bg-gradient-to-r from-cyber-card via-cyber-surface to-cyber-card flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center space-x-2">
              {userType === 'ADMIN' ? (
                <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-amber-500/15 text-amber-300 border border-amber-500/30">
                  PAINEL ADMINISTRATIVO • {currentAdmin.name.toUpperCase()}
                </span>
              ) : (
                <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-cyber-cyan/15 text-cyber-cyan border border-cyber-cyan/30">
                  PORTAL DO DENTISTA • {currentDentist.name.toUpperCase()}
                </span>
              )}
            ...
            <h1 className="text-lg sm:text-xl font-bold text-white">
              {userType === 'ADMIN'
                ...
                : `Seus Casos Cirúrgicos & Liberação de Guias 3D`}
            </h1>
          </div>

          <div className="flex items-center space-x-2 w-full sm:w-auto">
            {pendingCount > 0 && (
              <div className="px-3.5 py-2 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-300 text-xs font-mono flex items-center space-x-2">
        ...
        {/* Abas de Navegação */}
        <div className="flex items-center space-x-2 border-b border-cyber-border pb-3 overflow-x-auto">
          {/* Aba de Casos */}
          <button
            onClick={() => setActiveTab('CASES')}
            className={`px-4 py-2 rounded-xl text-xs font-semibold flex items-center space-x-2 transition-all whitespace-nowrap ${
              activeTab === 'CASES'
                ? 'bg-cyber-surface border border-cyber-cyan/40 text-cyber-cyan shadow-sm'
          ...
          >
            ...
            <span>Visualizador 3D WebGL</span>
          </button>
        </div>
        ...
        {activeTab === 'DEMO_3D' && (
          <div className="space-y-4">
            <div className="p-4 rounded-2xl glass-panel border border-cyber-border flex items-center justify-between">
              <div>
                <h3 className="text-sm font-bold text-white flex items-center space-x-2">
                  <Box className="w-4 h-4 text-cyber-cyan" />
                  <span>Ambiente de Demonstração Anatômica 3D</span>
                </h3>
                <p className="text-xs text-slate-400 font-mono mt-0.5">
                  Renderização interativa de mandíbula, implantes rosqueados nos dentes 36 e 46 e guia cirúrgica em resina translúcida
                </p>
              </div>
            </div>

            <DentalViewer3D
              patientName="Caso Clínico Demonstrativo"
              caseCode="IMP-DEMO-3D"
              isUnlocked={true}
            />
          </div>
        )}
      ...
      {/* Footer */}
      <footer className="border-t border-cyber-border py-4 mt-12 bg-cyber-surface/40">
